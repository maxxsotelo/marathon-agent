"""
fetch_run_deep_telemetry_aug5.py
Deep telemetry for the August 5 track run (typhoon override session).
"""
import os, sys, json
...
try:
    ...
    ae_te = details.get('aerobicTrainingEffect')
    an_te = details.get('anaerobicTrainingEffect')
    print(f"  Training Effect: Ae {ae_te} / An {an_te}")
...
try:
    ...
    humid  = weather.get('relativeHumidity', 'N/A')
    feels  = weather.get('apparentTemperatureInCelcius', weather.get('apparentTemperature', 'N/A'))
    print(f"  Temp: {temp}°C | Feels Like: {feels}°C | Humidity: {humid}%")
    print()
except Exception as e:
...
try:
    pr_data = run_act.get('pr', [])
    print("=== PERSONAL RECORDS (Garmin) ===")
    if pr_data:
        for pr in pr_data:
            print(f"  PR: {pr}")
    else:
        print("  No PR flags from Garmin API (check Garmin Connect manually).")
    print()
except Exception as e:
