...
acts = client.get_activities_by_date("2026-07-10", "2026-07-10")
print(f"Activities on 2026-07-10: {len(acts)} found\n")
for a in acts:
    ...
    avg_pace = a.get("averageSpeed", 0) or 0
    kcal = a.get("calories", 0) or 0
    elev_gain = a.get("elevationGain", 0) or 0
    ...
    print(f"  Elevation Gain: {elev_gain:.0f} m | Avg Cadence: {avg_cad:.0f} spm")

    if "running" in sport or "walking" in sport:
        print(f"\n  Fetching lap data...")
        try:
            splits = client.get_activity_splits(str(act_id))
            laps = splits.get("lapDTOs", [])
            print(f"  Lap  Dist(m)   Pace      AvgHR   MaxHR   Cad     EleG  EleL")
            print(f"  " + "-"*70)
            for i, lap in enumerate(laps, 1):
                ...
                lap_elev_g = lap.get("elevationGain", 0) or 0
                lap_elev_l = lap.get("elevationLoss", 0) or 0
                if lap_dist > 0 and lap_dur > 0:
                    ...
                    pace_str = "--:--"
                print(f"  {i:<4} {lap_dist:<9.0f} {pace_str:<9} {lap_hr:<7.0f} {lap_max_hr:<7.0f} {lap_cad:<7.0f} +{lap_elev_g:.0f}m  -{lap_elev_l:.0f}m")
